...
## §5.18 全模块稳健性检查（61/61，本会话收尾时跑的）

对 `--list-modules` 里的**每一个**模块单独跑一遍，看退出码与输出里有没有 panic：
...
- **未知模块**：`--module 不存在的模块` 报 `未知模块 …；可用：…`（61 个都列出来）——
  不静默、不部分渲染。
- **坏配置**：TOML 解析失败时给出**带行列号**的报错（`TOML parse error at line 1, column 13`
  加一段带 `^` 的上下文），不是一句「配置错误」。合法配置（`[[modules]]` + `type = "gpu"`）
  能正常只渲染这一行。

> 这一节的用法：改完任何模块之后，跑一遍这个循环比只看 `cargo test` 更能抓到「注册了但
> 一跑就炸」的情况——测试覆盖的是采集器的返回形状，不是整条 CLI 路径。
