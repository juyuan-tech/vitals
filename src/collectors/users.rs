...
impl Zone {
    ...
    fn format(&self, seconds: i64) -> Option<String> {
        let offset = match self {
            Self::Utc => 0,
            Self::File(bytes) => tzif_offset(bytes, seconds)?,
        };
...
/// （没有时间，而不是一个错的偏移）。
fn tzif_offset(bytes: &[u8], timestamp: i64) -> Option<i32> {
...
mod tests {
    ...
    fn a_tzif_file_gives_the_offset_for_that_moment() {
        ...
        // 第一个转换之前：用第一个非夏令时类型。
        assert_eq!(tzif_offset(&bytes, -2_000_000_000), Some(3600));
        assert_eq!(tzif_offset(&bytes, 0), Some(3600));
        assert_eq!(tzif_offset(&bytes, 1_000_000_000), Some(7200), "边界算在内");
        assert_eq!(tzif_offset(&bytes, 2_000_000_000), Some(7200));
    }
    ...
    fn a_v1_only_file_still_works() {
        ...
        assert_eq!(tzif_offset(&bytes, 5), Some(19800));
    }

    #[test]
    fn broken_timezone_data_yields_no_time_instead_of_a_wrong_one() {
        assert_eq!(tzif_offset(&[], 0), None);
        assert_eq!(tzif_offset(b"not a tzif file at all", 0), None);

        // 头部说是 TZif，但数据块被截断了：连第一块都读不全。
        let mut truncated = tzif(&[0], &[(3600, false)]);
        truncated.truncate(TZIF_HEADER + 9);
        assert_eq!(tzif_offset(&truncated, 0), None);
    }
