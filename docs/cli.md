...
| 顺序 | 来源 | 说明 |
| --- | --- | --- |
| 1 | `VITALS_LANG` | 只认 `zh*` 与 `en*`；写别的（包括打错字）**当没写**，继续往下看 |
| 2 | `LC_ALL` | |
| 3 | `LC_MESSAGES` | |
| 4 | `LANG` | |

locale 只取语言主体：`zh_CN.UTF-8` → `zh`、`en-US` → `en`。**拿不准一律中文**——
变量没设、值是 `C` / `POSIX`、或者压根不成语言标签时都算拿不准，所以「什么都不设」
和以前完全一样，不会因为换了台机器就变脸。除中文外的其它语言（例如 `de_DE.UTF-8`）
没有译文，退回英文。
